...
void CarSeeder::populateSpecs(QSqlDatabase &db) {
  ...
  while (sel.next()) {
    ...
    const CarSpecs determinedSpecs = determineSpecs(brand, model, suvKeys);

    QSqlQuery up(db);
    up.prepare("UPDATE cars SET engine_type=?, engine_capacity=?, power_hp=?, "
               "seats=? WHERE id=?");
    up.addBindValue(determinedSpecs.type);
    up.addBindValue(determinedSpecs.capacity);
    up.addBindValue(determinedSpecs.power);
    up.addBindValue(determinedSpecs.seats);
    up.addBindValue(id);
    up.exec();
  }
}
...
bool CarSeeder::insertCar(QSqlDatabase &db, const CarData &data) {
  QSqlQuery query(db);
  query.prepare(
      "INSERT INTO cars (brand, model, year, price_per_day, quantity, "
      "description, image_path) VALUES (?, ?, ?, ?, ?, ?, ?)");
  query.addBindValue(data.brand);
  query.addBindValue(data.model);
  query.addBindValue(data.year);
  query.addBindValue(data.pricePerDay);
  query.addBindValue(qBound(1, data.quantity, 10));
  query.addBindValue(data.description);
  query.addBindValue(data.imagePath);
  return query.exec();
}
