...
bool SchemaManager::ensureSchema(const QSqlDatabase &db) const {
  if (!createTables(db))
    return false;
  createIndexes(db);
  return true;
...
bool SchemaManager::createTables(const QSqlDatabase &db) const {
  QSqlQuery q(db);
...
void SchemaManager::createIndexes(const QSqlDatabase &db) const {
  QSqlQuery uq(db);
